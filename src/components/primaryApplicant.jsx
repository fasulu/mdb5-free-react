...
import {
    MDBContainer,
    MDBCard, MDBCardBody,
    MDBCardHeader, MDBCardTitle, MDBCardText, MDBCardFooter,
    MDBRow, MDBCol,
    MDBTypography,
    MDBBtn,
...
export default function PrimaryApplicant() {
    ...
    const [showLoginRef, setShowLoginRef] = useState(false);
    const [showNewClientDataEntry, setShowNewClientDataEntry] = useState(true);
    ...
    const findPostcodeAddress = (e) => {
        e.preventDefault();
        setModalInfo("Sorry... Postcode auto-fill not available rightnow, please enter your address manually")
        setShowInfoModal(true);
        setShowAddress(true);
    }
    ...
    const getThisClientID = async () => {
        ...
        try {
            ...
            if (response.data.message === "NINO registerd") {
    ...
    // create login reference number after new client signup
    const createLoginReference = async () => {

        // console.log(nINO, fName, sName, password, timeStampedMemorableDate)
        ...
        try {
            ...
            if (response.data.Status_Reply === "Success") {
                ...
                setLoginRef(response.data.ClientRef);
                console.table(response.data.ClientRef);
                setModalInfo(`Your Login Reference is ${response.data.ClientRef.toUpperCase()}.Please note down this reference number.\nYou will need this number to login when you visit next time.`)
                setShowInfoModal(true);
                setShowLoginRef(true);
                setShowNewClientDataEntry(false);
            }
    ...
    const ShowLoginRefInfo = (
        <React.Fragment>
            <MDBContainer className='ps-5 pt-3' autoFocus={true} >
                <MDBCard alignment='center' shadow='0' border='info' background='white' className='mb-3'>
                    <MDBCardHeader><strong> Your New Login Reference </strong>
                    ...
                    <MDBCardBody>
                        <MDBCardTitle style={{ fontSize: '20px', fontWeight: 'bold' }}>{loginRef.toUpperCase()}</MDBCardTitle>
                        <MDBCardText >Please note down this login reference. You need login reference and your password to login to this website to complete this application</MDBCardText>
                    </MDBCardBody>
                    <MDBCardFooter>
                        Admin, Housing Department
                    </MDBCardFooter>
                </MDBCard>
            </MDBContainer >
        </React.Fragment>
    ...
    return (
        <React.Fragment>
            ...
            {
                showLoginRef && ShowLoginRefInfo
            }

            {
                showNewClientDataEntry &&
